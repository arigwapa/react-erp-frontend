// ==========================================
// Sidebar.tsx
// Collapsible sidebar navigation for the Super Admin panel.
// Features:
//   - Expand/collapse toggle with chevron button
//   - Collapsible nav groups (dropdown sections)
//   - Auto-expands group containing the active route
//   - Role indicator badge below the logo
...
const menuGroups = [
  ...
  },
  {
    label: "Modules",
    items: [
      { icon: Layers, label: "PLM (Products)", path: "/admin/plm" },
      { icon: Factory, label: "Production", path: "/admin/production" },
      { icon: ClipboardCheck, label: "Quality (QA)", path: "/admin/quality" },
      { icon: Warehouse, label: "Warehouse", path: "/admin/warehouse" },
...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  // _onToggleCollapse is kept for interface compatibility (toggle lives in the navbar now)
  ...
  useEffect(() => {
    ...
    menuGroups.forEach((group) => {
      ...
      // Keep previously expanded groups open, auto-expand active group
      newExpanded[group.label] =
        hasActiveItem || expandedGroups[group.label] || false;
    });
    setExpandedGroups(newExpanded);
    // Only run when route changes, not on every expandedGroups change
                ...
                >
                  {group.items.map((item, itemIdx) => (
                    ...
                    >
                      ...
                      {/* Badge dot — shown when collapsed and badge exists */}
                      {item.badge && isCollapsed && (
