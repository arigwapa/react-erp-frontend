// ==========================================
// FinanceLayout.tsx
// Top-level layout wrapper for all Finance Manager pages.
// Mirrors other role layouts but uses FinanceSidebar
// and displays "Finance Manager" in the header.
// Branch-scoped layout — Manila branch.
// Theme color: Emerald (from-emerald-500 to-emerald-600)
// ==========================================
...
const FinanceLayout: React.FC<FinanceLayoutProps> = ({ children }) => {
  ...
  const SEGMENT_LABELS: Record<string, string> = {
    ...
    "spending-limits": "Spending Limits & Thresholds",
    "cost-records": "Auto-COGS (Cost Records)",
    "cogs-review": "COGS Review by Work Order",
    profitability: "Profitability Analysis",
    "cost-validation": "Cost Entry Validation",
    variance: "Variance & Exceptions",
    "period-locking": "Period Locking",
        ...
        <header className="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-700 sticky top-0 z-40 pl-0 pr-4 sm:pr-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
              <Link to="/finance-manager/dashboard" className="hover:text-indigo-600 transition-colors flex items-center gap-1"><Home size={16} /></Link>
          ...
          <div className="flex items-center gap-3 pl-6 border-l border-slate-200 dark:border-slate-700 h-10 my-auto">
            <div className="text-right hidden md:block">
              <p className="text-sm font-bold text-slate-800 dark:text-slate-200">Finance Manager</p>
